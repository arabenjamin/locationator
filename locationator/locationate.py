...
class Locationator():
    ...
    def get_zip_frm_addr(self,address):
        """ Get the zip code from an address string that doesn't include the zip """
        # Make sure it's not nothing
        if address is None or len(address) == 0:
            raise EmptyParameterError("Won't work with an empty address")
        # make sure it's the right type
        ...
        url = self.google_base_url + path
        try:
            return requests.get(url, headers = self.headers).json()['results'][0]['address_components'][-1:][0]['short_name']
        except RequestException:
            raise NetworkError('Requests ran into a problem with your HTTP request')
